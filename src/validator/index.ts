...
function buildItemSchemasForSlice(
    vctx: ValidationContext,
    sliceSchema?: FHIRSchemaElement,
): Record<string, FHIRSchema> {
                ...
                if (slicing?.slices && Object.keys((slicing as any).slices).length > 0) {
                    ...
                    if (cls.error === 'ambiguous') {
                        ...
                        const itemSchemas = buildItemSchemasForSlice(
                            vctx,
                            ((slicing as any).slices as any)[cls.slice]
                                ?.schema as FHIRSchemaElement,
                        );
