 ...
 */
/**
 * Whether a package's prepared fixtures exist locally (test/fixtures/<pkg>/).
 * Lets data-driven suites skip — rather than fail — cases whose fixtures were
 * not produced by `bun run prepare-fixtures` in this environment.
 */
export function packageFixturesAvailable(pkgId: string): boolean {
  return existsSync(join(FIXTURES_ROOT, pkgId));
}
...
/** Whether the external HL7 fhir-test-cases checkout is present (for _loadProfile cases). */
export function fhirTestCasesAvailable(): boolean {
  return existsSync(FHIR_TEST_CASES_ROOT);
}
