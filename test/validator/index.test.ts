...
import { type AtomicContext, validateSchema, FHIRSchemaErrorCode } from '../../src/validator';
...
function resolveSchema(_ctx: AtomicContext, url: string) {
...
describe('validator', () => {
    it('test basic navigation', () => {
        ...
        expect(res2.errors).toMatchObject([
            { code: FHIRSchemaErrorCode.UnknownElement, path: 'Simple.ups' },
        ]);

        const res3 = validateSchema(ctx, {
            schemaUrls: [],
            resource: { resourceType: 'Ups' },
        });
        expect(res3.errors).toMatchObject([
            { code: FHIRSchemaErrorCode.UnknownSchema, path: 'Ups.resourceType' },
        ]);
    });
    ...
    it('test array', () => {
        ...
        expect(res1.errors).toMatchObject([
            { code: FHIRSchemaErrorCode.ExpectedArray, path: 'WithArray.a' },
        ]);
        ...
        expect(res2.errors).toMatchObject([
            { code: FHIRSchemaErrorCode.UnexpectedArray, path: 'WithArray.b' },
        ]);
    });
    ...
    it('test nested elements', () => {
        ...
        expect(res0.errors).toMatchObject([
            { code: FHIRSchemaErrorCode.UnknownElement, path: 'Simple.c.ups' },
        ]);
    ...
    it('multiple schemas', () => {
        ...
        expect(res1.errors).toMatchObject([
            { code: FHIRSchemaErrorCode.UnknownElement, path: '.ups' },
        ]);
    });
    ...
    it('backbone element', () => {
        ...
        const res1 = validateSchema(ctx, {
            schemaUrls: [],
            resource: {
                resourceType: 'WithBackbone',
                a: { b: 1, extension: [{ url: 'exta', valueString: 'string' }] },
            },
        });
        expect(res1.errors).toEqual([]);
    ...
    it('slicing: slice min cardinality violation is reported', () => {
        ...
        expect(res.errors).toMatchObject([{ code: 'FS009' }]);
    });
    ...
    it('slicing: closed slicing errors on unmatched items', () => {
        ...
        expect(res.errors).toMatchObject([{ code: 'FS007', path: 'SliceClosed.x.0' }]);
    });
