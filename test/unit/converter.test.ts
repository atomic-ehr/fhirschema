...
import { translate, parsePath, calculateActions, getCommonPath } from '../../src/converter';
...
describe('Converter Algorithm Tests', () => {
  ...
  describe('calculateActions', () => {
    it('should calculate basic actions', () => {
      expect(calculateActions(parsePath({ path: 'R.a' }), parsePath({ path: 'R.b' }))).toEqual([
        { type: 'exit', el: 'a' },
        { type: 'enter', el: 'b' },
      ]);

      expect(calculateActions(parsePath({ path: 'R.a' }), parsePath({ path: 'R.b.c' }))).toEqual([
        { type: 'exit', el: 'a' },
        { type: 'enter', el: 'b' },
        { type: 'enter', el: 'c' },
      ]);

      expect(calculateActions(parsePath({ path: 'R.a.b.c' }), [])).toEqual([
        { type: 'exit', el: 'c' },
        { type: 'exit', el: 'b' },
        { type: 'exit', el: 'a' },
      ]);
